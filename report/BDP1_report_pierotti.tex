...
Docker containers (\texttt{https://www.docker.com/}) can be used to easily ship and deploy applications.
In this section I repeated the same task already accomplished using HTCondor and the BWA tool, but this time encapsulating the application in a Docker container.
HTCondor was used also in this section to orchestrate the execution of batch jobs.
The HTCondor \texttt{docker} universe was used instead of the \texttt{vanilla} universe.
...
I installed Docker on it with the command \texttt{sudo apt install docker.io} and I added the current user to the \texttt{docker} group with the command \texttt{sudo usermod -aG docker \$USER}.
I added the user \texttt{condor} to the \texttt{docker} group with the command \texttt{sudo usermod -aG docker condor}.
In order to make the shared NFS \texttt{/data} volume accessible to Docker containers, the file \texttt{/etc/condor/condor\_config} was modified by appending the following lines:
...
I created a new AMI from it, so to be able to replicate the Worker Node instances as required.
...
I modified the test file job file in order to use the \texttt{docker} universe instead of the \texttt{vanilla} universe, and I specified into it the Docker image to be used
