...
Ten instances of this test job were run on the cluster of 1 Master Node and 2 Worker Nodes.
...
The general model followed would be that of transferring the executables and the input fasta files using the HTCondor Input Sandbox.
...
\section{Creation of a Container Image for the Alignment of NGS reads}
...
A plausible non-trivial use-case that can be addressed using the HTCondor cluster developped in Section 1 is the allignment of a big number of fasta files to the human reference genome hg19.
...
This task consists in aligning NGS 554000 reads from one individual against the hg19 genome.
Given that a single NGS-produced fasta file like the ones used for testing the infrastructure typically contains 1000 reads, the task consists in aligning 554 fasta files.
From the tests performed, I roughly determined that the time needed to align one fasta file is around 227.3 seconds.
...
If I assume that I will use the cluster detailed in this project, with 2 Worker Nodes and one Master Node, I will have four cores at my disposal (two per Worker Node).
